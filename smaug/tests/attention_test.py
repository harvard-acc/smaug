...
class AttentionTest(TensorflowTest):
  ...
  def test_bahdanau_attention(self):
    ...
    query = Tensor(data_layout=NC, tensor_data=query.numpy())
    w, u = recurrent_test.createSmaugWeights(tf_cell)
    memory = Tensor(data_layout=NTC, tensor_data=memory.numpy())
    weights = attention_mechanism.get_weights()
    w_alignment = Tensor(
        data_layout=NC, tensor_data=np.expand_dims(weights[0], 0))
    w_decoder = Tensor(data_layout=NC, tensor_data=np.transpose(weights[1]))
    w_encoder = Tensor(data_layout=NC, tensor_data=np.transpose(weights[2]))
    with Graph(name=self.graph_name, backend=self.backend) as graph:
      ...
      sg_initial_attention = Tensor(
          data_layout=NC,
          tensor_data=np.zeros((batch, units), dtype=self.dtype))
