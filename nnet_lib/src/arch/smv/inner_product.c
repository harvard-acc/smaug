...
void inner_product_layer_smv_hw_impl(float* dma_activations,
                                     float* dma_weights,
                                     float* dma_results,
...
void inner_product_layer_smv_hw(float* dma_activations,
                                float* dma_weights,
                                float* dma_results,
...
#define INNER_PROD_NO_DMA_IMPL(INPUT, WGT, LR)                                 \
    do {                                                                       \
        ...
        inner_product_layer_smv_hw_impl(NULL, NULL, NULL, INPUT##_activations, \
...
#define INNER_PROD_WITH_DMA_IMPL(HA, HW, HR, LA, LW, LR)                       \
    do {                                                                       \
        ...
        inner_product_layer_smv_hw_impl(HA##_activations, HW##_weights,        \
...
fc_cfg_t inner_product_smv_tile_rowwise(layer_t* curr_layer) {
    fc_cfg_t fc_cfgs;
...
void inner_product_layer_smv_hw_dispatch(float* activations,
                                         float* weights,
                                         float* results,
                                         layer_t* layer,
                                         int result_size,
...
void inner_product_layer_smv_impl_rowwise(float* host_activations,
                                          float* host_weights,
                                          layer_t* curr_layer,
                                          float* host_results,
                                          device_t* device,
    ...
    fc_cfg_t fc_cfgs = inner_product_smv_tile_rowwise(curr_layer);
    ...
    for (unsigned it = 0; it < fc_cfgs.num_iterations; it++) {
        ...
        if (curr_layer->wgt_storage_type == PackedCSR) {
            decompress_packed_csr_smiv_impl(
                    &partial_layer, current_row, input_in_spad0, device);
        ...
        inner_product_layer_smv_hw_dispatch(host_activations,
                                            curr_dense_weights_loc,
                                            host_results,
                                            &partial_layer,
                                            result_size,
...
void inner_product_layer_smv_impl(float* host_activations,
    ...
    inner_product_layer_smv_impl_rowwise(host_activations,
                                         host_weights_layer,
                                         curr_layer,
                                         host_results,
                                         device,
