...
TEST_CASE_METHOD(SmaugTest, "Reference batch norm operator", "[refop]") {
    ...
    SECTION("Batch norm after convolution") {
        ...
        allocateAllTensors<float, ReferenceBackend>(bnOp);
        auto meanTensor = bnOp->getInput<ReferenceBackend>(1);
        auto varianceTensor = bnOp->getInput<ReferenceBackend>(2);
        auto gammaTensor = bnOp->getInput<ReferenceBackend>(3);
        auto betaTensor = bnOp->getInput<ReferenceBackend>(4);
        ...
        std::vector<float> expectedValues{ -5,-10,-15,-20,  // chan 1
                                           ...
                                           24,-8,-40,-72,
                                           -8,-40,-72,-104 };
        auto outputsTensor = bnOp->getOutput<ReferenceBackend>(0);
