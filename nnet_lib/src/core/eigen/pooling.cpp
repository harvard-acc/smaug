...
void max_pooling_colmajor(float* activations, float* result, layer_t curr_layer) {
    ...
    // The patches get extracted interleaved across channels (unsure about
    // input batching right now), so this shuffle operation undoes the effect
    // of that.
...
void max_pooling(float* activations, float* result, layer_t curr_layer) {
    max_pooling_rowmajor(activations, result, curr_layer);
}
