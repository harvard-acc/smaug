...
void activation_fun(float* activations,
    ...
    if (function == RELU) {
        session->add_op(relu(activations, total_size, session, results, 0));
    } else if (function == SIGMOID) {
        session->add_op(sigmoid(activations, total_size, session, results));
    } else if (function == LRELU) {
        static const float alpha = 0.1;
        session->add_op(relu(activations, total_size, session, results, alpha));
    } else if (function == ELU) {
        session->add_op(elu(activations, total_size, session, results));
    } else if (function == SELU) {
        session->add_op(selu(activations, total_size, session, results));
    } else if (function == TANH) {
        session->add_op(tanh(activations, total_size, session, results));
    } else if (function == SOFTMAX) {
        session->add_op(
                softmax(activations, batch_size, input_size, session, results));
    } else {
